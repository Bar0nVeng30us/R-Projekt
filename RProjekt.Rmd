...
an Tag t+1 schwerer sind, als an Tag t. Ist dies an Tag t nicht der Fall, 
so setzen wir das Gewicht von Tag t (g(t)) auf den Durchschnitt der Nachbartage 
((g(t-2) + g(t+2))/2).Für einzelne Außreißer funktioniert dieses System gut, 
sind allerdings mehrere Tage in Folge falsch, so müsste man eine andere Variante wählen.
...
### 2.b)
Wir nutzen eine Dummy-Kodierung für die Ernährungspläne. Dabei ist Plan 1 der Standardfall mit einer Standardfehler von ca. 3.3. Die anderen 3 Pläne haben einen Standardfehler um die 4.1 . Unser Modell scheint gut zu funktionieren, da der p-Wert nahe der Maschinengenauigkeit liegt. Allerdings scheint es starke Streuungen zu geben, da die Entfernung zwischen minimalem und maximalem Residuum sehr groß ist (dies wird später noch durch die Grafik bestätigt werden, in der wir sehen, dass die Gewichtswerte zum Ende der Messungen hin stark variieren.
