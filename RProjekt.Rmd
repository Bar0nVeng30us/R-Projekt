...
adjust_weight <- function(v, tolerated_deviation = 30)
{
  ...
  extreme_vals[c(2:length(extreme_vals))] <- vapply(c(2:length(extreme_vals)), FUN = function(i) {
    if (extreme_vals[i] == TRUE && extreme_vals[i-1] == TRUE) {
      return(FALSE)}
    else {return(extreme_vals[i])}},
    FUN.VALUE = logical(1))
